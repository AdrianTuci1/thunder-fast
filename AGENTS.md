...
- **Atenție bidirecțională:** `make_bidirectional` este best-effort pentru familia Llama/Qwen;
  **verifică** pe arhitectura exactă aleasă (HF variază între versiuni). Dacă nu se poate dezactiva
  ...
  testată.
- **Program de mascare (discret MDM):** raportul % de poziții mascate, curriculum-ul
  `[mask_ratio_min, mask_ratio_max]` și unmask-ul progresiv trebuie să coincidă între
  `MaskedDiffusion` și bucla `generate()`; nu folosi zgomot Gaussian continuu (respins în ADR 0008).
...
1. **Încărcare model 0.6B + conversie atenție bidir + `[MASK]`** — `DiffusionLM` se inițializează
   cu masca discretă adăugată (verifică `mask_token_id` valid, fără segfault/OOM).
...
5. **Generare/decodare** — `eval.py` rulează `generate()` la 24 pași și decodează tokeni (nu trebuie
...
- **Program de mascare / difuzie** — `src/train/diffusion.py` (`MaskedDiffusion`). Adaugă variante
  noi (ex. curriculum diferențiat de mascare, algoritmi de unmask p2/entropy/gen, re-mascare)
  fără a atinge modelul.
- **Model / backbone** — `src/train/model.py` (`DiffusionLM`). Schimbă arhitectura de bază (Qwen/Llama/Gemma)
  sau token-ul mască / unmask-ul progresiv doar aici.
...
`MaskedDiffusion` e singura sursă de adevăr pentru programul de mascare, folosit și în `generate()`).
